...
// Build drops reference table from enemy data
const DROPS_TABLE = {};
ENEMIES.forEach(enemy => {
  enemy.drops.forEach(drop => {
    if(!DROPS_TABLE[drop]) DROPS_TABLE[drop] = [];
    DROPS_TABLE[drop].push(enemy.name);
  });
});
const DROPS_SORTED = Object.entries(DROPS_TABLE).sort((a,b) => a[0].localeCompare(b[0]));
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:1100,margin:"0 auto",padding:"24px 20px"}}>
        ...
        <div style={{textAlign:"center",marginBottom:18}}>
          <div style={{display:"inline-block",border:`1px solid ${C.cyan}33`,borderTop:`2px solid ${C.cyan}`,padding:"3px 18px",marginBottom:5}}>
            <span style={{fontSize:15,letterSpacing:5,color:`${C.cyan}88`}}>SPERANZA TACTICAL DATABASE · ALL MAPS</span>
          </div>
          <h1 style={{fontSize:25,fontWeight:900,letterSpacing:3,margin:"5px 0 3px",color:C.textBright,textTransform:"uppercase"}}>ARC RAIDERS FIELD GUIDE</h1>
          <p style={{fontSize:15,color:C.textDim,letterSpacing:2,margin:0}}>COMPLETE LOOT ({ALL_ITEMS.length} ITEMS) · QUEST WALKTHROUGHS · WORKSHOP — FLASHPOINT UPDATE</p>
        </div>

        {/* Main section tabs */}
        <div style={{display:"flex",gap:3,marginBottom:14}}>
          {SECTIONS.map(sec=>(
            <button key={sec} onClick={()=>{setSection(sec);setSearch("");setExpanded(null);setDecFilter("all");setTierFilter("All");}} style={{
              ...
              cursor:"pointer",fontSize:17,letterSpacing:4,...F,fontWeight:section===sec?700:400,
            }}>{sec}</button>
          ))}
        </div>
        ...
        <div style={{marginBottom:12,position:"relative"}}>
          ...
          <input value={search} onChange={e=>{setSearch(e.target.value);setExpanded(null);}}
            ...
            style={{width:"100%",boxSizing:"border-box",background:"#0c1215",border:`1px solid ${C.borderBright}`,borderLeft:`2px solid ${C.cyan}44`,color:"#a8d8e8",padding:"8px 10px 8px 26px",fontSize:16,letterSpacing:1,outline:"none",...F}}/>
        </div>
        ...
        {section==="LOOT" && (
          <>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:5,marginBottom:12}}>
              <div onClick={()=>setDecFilter("all")} style={{padding:"10px 12px",cursor:"pointer",border:`1px solid ${decFilter==="all"?C.cyan:C.border}`,borderTop:`2px solid ${decFilter==="all"?C.cyan:"transparent"}`,background:decFilter==="all"?"#0c1a14":C.panel}}>
                <div style={{fontSize:17,letterSpacing:2,color:decFilter==="all"?C.cyan:C.dim}}>ALL</div>
                <div style={{fontSize:16,fontWeight:700,color:C.textBright}}>{ALL_ITEMS.length}</div>
              </div>
              {Object.entries(DECISIONS).map(([key,d])=>(
                <div key={key} onClick={()=>setDecFilter(decFilter===key?"all":key)} style={{padding:"10px 12px",cursor:"pointer",border:`1px solid ${decFilter===key?d.color:C.border}`,borderTop:`2px solid ${decFilter===key?d.color:"transparent"}`,background:decFilter===key?d.bg:C.panel}}>
                  <div style={{fontSize:17,letterSpacing:1,color:decFilter===key?d.color:C.dim}}>{d.label}</div>
                  <div style={{fontSize:16,fontWeight:700,color:C.textBright}}>{ALL_ITEMS.filter(i=>i.decision===key).length}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
              <select value={rarFilter} onChange={e=>setRarFilter(e.target.value)} style={{flex:"1 1 120px",background:"#0c1215",border:`1px solid ${C.border}`,color:C.textDim,padding:"10px 12px",fontSize:16,...F,outline:"none"}}>
                {RARITIES.map(r=><option key={r} value={r}>{r==="All"?"All Rarities":r}</option>)}
              </select>
              <select value={catFilter} onChange={e=>setCatFilter(e.target.value)} style={{flex:"1 1 140px",background:"#0c1215",border:`1px solid ${C.border}`,color:C.textDim,padding:"10px 12px",fontSize:16,...F,outline:"none"}}>
                {CATS.map(c=><option key={c} value={c}>{c==="All"?"All Categories":c}</option>)}
              </select>
            </div>

            <div style={{fontSize:15,color:C.dim,letterSpacing:2,marginBottom:8}}>SHOWING {filteredLoot.length} OF {ALL_ITEMS.length} ITEMS</div>

            {/* Desktop table — hidden on mobile via container, cards shown instead */}
            <style>{`
              @media (max-width: 640px) {
                .loot-table { display: none !important; }
                .loot-cards { display: flex !important; }
              }
              @media (min-width: 641px) {
                .loot-table { display: block !important; }
                .loot-cards { display: none !important; }
              }
            `}</style>

            {/* ── Desktop table ── */}
            <div className="loot-table" style={{border:`1px solid ${C.border}`}}>
              <div style={{display:"grid",gridTemplateColumns:"170px 110px 100px 95px 1fr",background:"#0c1115",borderBottom:`1px solid ${C.border}`}}>
                {["ITEM","RARITY","CATEGORY","ACTION / QTY","WHY / DETAILS"].map(h=>(
                  <div key={h} style={{padding:"7px 12px",fontSize:11,letterSpacing:2,color:C.dimmer,borderRight:`1px solid ${C.border}`,overflow:"hidden",overflowWrap:"break-word"}}>{h}</div>
                ))}
              </div>
              {filteredLoot.length===0 && <div style={{padding:24,textAlign:"center",color:C.dim,fontSize:17,letterSpacing:3}}>NO ITEMS MATCH</div>}
              {filteredLoot.map((item,i)=>{
                const d=DECISIONS[item.decision];
                return (
                  <div key={i} style={{display:"grid",gridTemplateColumns:"170px 110px 100px 95px 1fr",borderBottom:i<filteredLoot.length-1?`1px solid #0f1820`:"none",background:i%2===0?"#090c0e":"#0b0e11"}}
                    ...
                    <div style={{padding:"10px 12px",fontSize:13,color:C.textBright,display:"flex",alignItems:"center",gap:5,borderRight:`1px solid #0f1820`,minWidth:0,overflow:"hidden"}}>
                      <span style={{width:3,height:3,background:d.color,flexShrink:0,display:"inline-block"}}/>
                      <span style={{overflowWrap:"break-word",wordBreak:"break-word"}}>{item.name}</span>
                    </div>
                    <div style={{padding:"10px 12px",fontSize:13,color:RARE_COLORS[item.rarity]||C.dim,borderRight:`1px solid #0f1820`,display:"flex",alignItems:"center",minWidth:0,whiteSpace:"nowrap"}}>{item.rarity}</div>
                    <div style={{padding:"10px 12px",fontSize:13,color:C.textDim,borderRight:`1px solid #0f1820`,display:"flex",alignItems:"center",minWidth:0,overflow:"hidden",overflowWrap:"break-word",wordBreak:"break-word"}}>{item.cat}</div>
                    <div style={{padding:"10px 12px",borderRight:`1px solid #0f1820`,display:"flex",flexDirection:"column",gap:3,justifyContent:"center",minWidth:0,overflow:"hidden"}}>
                      <span style={{fontSize:12,letterSpacing:1,color:d.color,background:d.bg,border:`1px solid ${d.color}44`,padding:"2px 4px",textAlign:"center",overflowWrap:"break-word"}}>{d.label}</span>
                      {item.qty && item.qty!=="–" && <span style={{fontSize:13,color:C.gold,textAlign:"center",fontWeight:700,overflowWrap:"break-word"}}>{item.qty}</span>}
                    </div>
                    <div style={{padding:"10px 14px",fontSize:13,color:C.textDim,lineHeight:1.7,overflowWrap:"break-word",wordBreak:"break-word",minWidth:0}}>{item.note}</div>
                  </div>
                );
              })}
            </div>
            ...
            {/* ── Mobile cards ── */}
            <div className="loot-cards" style={{flexDirection:"column",gap:8,display:"none"}}>
              {filteredLoot.length===0 && <div style={{padding:24,textAlign:"center",color:C.dim,fontSize:15,letterSpacing:3}}>NO ITEMS MATCH</div>}
              {filteredLoot.map((item,i)=>{
                const d=DECISIONS[item.decision];
                return (
                  <div key={i} style={{background:C.panel,border:`1px solid ${C.border}`,borderLeft:`3px solid ${d.color}`,padding:"12px 14px"}}>
                    {/* Top row: name + badge */}
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginBottom:8,flexWrap:"wrap"}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{width:4,height:4,background:d.color,flexShrink:0,display:"inline-block"}}/>
                        <span style={{fontSize:14,color:C.textBright,fontWeight:700}}>{item.name}</span>
                      </div>
                      <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
                        <span style={{fontSize:11,color:RARE_COLORS[item.rarity]||C.dim}}>{item.rarity}</span>
                        <span style={{fontSize:11,letterSpacing:1,color:d.color,background:d.bg,border:`1px solid ${d.color}44`,padding:"2px 7px"}}>{d.label}</span>
                        {item.qty && item.qty!=="–" && <span style={{fontSize:12,color:C.gold,fontWeight:700}}>{item.qty}</span>}
                      </div>
                    </div>
                    {/* Category */}
                    <div style={{fontSize:11,color:C.dim,letterSpacing:1,marginBottom:6}}>{item.cat}</div>
                    {/* Details */}
                    <div style={{fontSize:13,color:C.textDim,lineHeight:1.7}}>{item.note}</div>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{marginTop:10,display:"flex",flexWrap:"wrap",gap:6}}>
              {Object.entries(DECISIONS).map(([key,d])=>(
                <div key={key} style={{display:"flex",alignItems:"center",gap:5,padding:"3px 7px",background:d.bg,border:`1px solid ${d.color}33`}}>
                  ...
                  <span style={{fontSize:17,letterSpacing:2,color:d.color}}>{d.label}</span>
                </div>
              ))}
              <span style={{fontSize:17,color:C.dimmer,letterSpacing:1,alignSelf:"center"}}>· ALWAYS RECYCLE AT SPERANZA, NOT MID-RAID · QTY = MINIMUM TO KEEP</span>
            </div>
          </>
        )}
        ...
        {section==="QUESTS" && (
          <>
            <div style={{fontSize:15,color:C.textDim,letterSpacing:2,marginBottom:10}}>
              ...
              {Object.keys(questData).map(n=>{
                ...
                return (
                  <button key={n} onClick={()=>{setNpc(n);setExpanded(null);}} style={{
                    ...
                    cursor:"pointer",fontSize:16,letterSpacing:2,...F,fontWeight:active?700:400,whiteSpace:"nowrap",
            ...
            <div style={{border:`1px solid ${questData[npc].color}33`,borderLeft:`3px solid ${questData[npc].color}`,padding:"7px 12px",marginBottom:12,background:"#0c1014"}}>
              <span style={{fontSize:16,color:questData[npc].color,letterSpacing:2,fontWeight:700}}>{npc.toUpperCase()} — </span>
              <span style={{fontSize:16,color:C.textDim}}>{questData[npc].role} · {questData[npc].focus}</span>
            </div>
                  ...
                  <div onClick={()=>setExpanded(open?null:key)}
                    ...
                    <div style={{flex:1}}>
                      <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
                        <span style={{fontSize:16,color:col,fontWeight:700}}>#{q.num}</span>
                        <span style={{fontSize:17,color:C.textBright,fontWeight:700}}>{q.name}</span>
                        <span style={{fontSize:17,letterSpacing:2,color:col,background:`${col}15`,border:`1px solid ${col}44`,padding:"2px 6px"}}>{q.map}</span>
                      </div>
                      <div style={{fontSize:16,color:C.textDim,lineHeight:1.5}}>{q.obj}</div>
                    </div>
                    <span style={{color:col,fontSize:17,flexShrink:0,marginTop:2}}>{open?"▲":"▼"}</span>
                  </div>
                  {open && (
                    <div style={{padding:"12px 14px"}}>
                      <div style={{marginBottom:10}}>
                        <div style={{fontSize:17,letterSpacing:3,color:col,marginBottom:4}}>📍 WHERE TO GO + FIND ITEMS</div>
                        <div style={{fontSize:16,color:C.text,lineHeight:1.7,background:`${col}08`,padding:"8px 10px",borderLeft:`2px solid ${col}44`}}>{q.where}</div>
                      </div>
                      {q.bring && (
                        <div style={{marginBottom:10}}>
                          <div style={{fontSize:17,letterSpacing:3,color:C.gold,marginBottom:4}}>🎒 BRING INTO RAID</div>
                          <div style={{fontSize:16,color:C.text,background:`${C.gold}08`,padding:"8px 10px",borderLeft:`2px solid ${C.gold}44`}}>{q.bring}</div>
                        </div>
                      )}
                      <div style={{marginBottom:10}}>
                        <div style={{fontSize:17,letterSpacing:3,color:C.green,marginBottom:4}}>STEP-BY-STEP</div>
                        <div style={{background:`${C.green}06`,padding:"8px 10px",borderLeft:`2px solid ${C.green}33`}}>
                          {q.steps.map((step,si)=>(
                            <div key={si} style={{display:"flex",gap:8,marginBottom:si<q.steps.length-1?5:0}}>
                              <span style={{color:C.green,fontSize:15,fontWeight:700,flexShrink:0,marginTop:1}}>{si+1}.</span>
                              <span style={{fontSize:16,color:C.text,lineHeight:1.6}}>{step}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
                        <div style={{background:`${C.orange}08`,padding:"8px 10px",borderLeft:`2px solid ${C.orange}44`}}>
                          <div style={{fontSize:17,letterSpacing:3,color:C.orange,marginBottom:4}}>⚡ PRO TIP</div>
                          <div style={{fontSize:16,color:C.textDim,lineHeight:1.6}}>{q.tip}</div>
                        </div>
                        <div style={{background:`${C.gold}08`,padding:"8px 10px",borderLeft:`2px solid ${C.gold}44`}}>
                          <div style={{fontSize:17,letterSpacing:3,color:C.gold,marginBottom:4}}>★ REWARD</div>
                          <div style={{fontSize:16,color:C.gold,lineHeight:1.6}}>{q.reward}</div>
                        </div>
                      </div>
                    </div>
                  )}
            ...
            {filteredQuests.length===0 && <div style={{textAlign:"center",color:C.dim,fontSize:17,padding:30,letterSpacing:3}}>NO RESULTS</div>}
          </>
        )}
        ...
        {section==="WORKSHOP" && (
          <>
            <div style={{fontSize:15,color:C.textDim,letterSpacing:2,marginBottom:12}}>
            ...
            {filteredWorkshop.map((st,si)=>(
              <div key={si} style={{marginBottom:18}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                  ...
                  <span style={{fontSize:15,letterSpacing:3,color:st.color,fontWeight:700}}>{st.name.toUpperCase()}</span>
                  <div style={{flex:1,height:1,background:`${st.color}22`}}/>
                </div>
                {st.note && <div style={{fontSize:16,color:C.textDim,marginBottom:7,paddingLeft:27,lineHeight:1.6}}>{st.note}</div>}
                <div style={{border:`1px solid ${C.border}`}}>
                  <div style={{display:"grid",gridTemplateColumns:"90px 1fr 1fr",background:"#0c1115",borderBottom:`1px solid ${C.border}`}}>
                    {["LEVEL","UNLOCKS","MATERIALS REQUIRED"].map(h=>(
                      <div key={h} style={{padding:"7px 12px",fontSize:17,letterSpacing:2,color:C.dimmer,borderRight:`1px solid ${C.border}`}}>{h}</div>
                    ))}
                  </div>
                  {st.levels.map((lvl,li)=>(
                    <div key={li} style={{display:"grid",gridTemplateColumns:"90px 1fr 1fr",borderBottom:li<st.levels.length-1?`1px solid #0f1820`:"none",background:li%2===0?"#090c0e":"#0b0e11"}}
                      ...
                      <div style={{padding:"10px 12px",fontSize:17,color:st.color,fontWeight:700,borderRight:`1px solid #0f1820`}}>{lvl.level}</div>
                      <div style={{padding:"10px 12px",fontSize:16,color:C.text,borderRight:`1px solid #0f1820`,lineHeight:1.5}}>{lvl.unlocks}</div>
                      <div style={{padding:"10px 12px",fontSize:16,color:C.textDim,lineHeight:1.5}}>{lvl.materials}</div>
                    </div>
                  ))}
                </div>
              </div>
            ))}
            {filteredWorkshop.length===0 && <div style={{textAlign:"center",color:C.dim,fontSize:17,padding:30,letterSpacing:3}}>NO RESULTS</div>}
          </>
        )}
          ...
          <>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:16}}>
              {["All", ...TIER_ORDER].map(tier=>{
                const col = tier === "All" ? C.cyan : TIER_COLORS[tier];
                ...
                return (
                  <button key={tier} onClick={()=>setTierFilter(tier)} style={{
                    ...
                    borderTop: active ? `2px solid ${col}` : `2px solid transparent`,
                    color: active ? col : C.dim,
                    fontSize:12,letterSpacing:2,...F,fontWeight:active?700:400,
                    whiteSpace:"nowrap",
                  }}>{tier.toUpperCase()}</button>
                );
              })}
            </div>
            ...
            {TIER_ORDER.map(tier=>{
              ...
              // ── LOOT OBJECT tier = drops reference table ──
              if(tier === "Loot Object") {
                const filtered = DROPS_SORTED.filter(([drop]) =>
                  !search || drop.toLowerCase().includes(search.toLowerCase()) ||
                  DROPS_TABLE[drop].join(" ").toLowerCase().includes(search.toLowerCase())
                );
                ...
                return (
                  <div key={tier} style={{marginBottom:20}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                      ...
                      <span style={{fontSize:15,letterSpacing:4,color:col,fontWeight:700}}>LOOT DROPS REFERENCE</span>
                      <div style={{flex:1,height:1,background:`${col}22`}}/>
                      <span style={{fontSize:13,color:C.dimmer,letterSpacing:2}}>{filtered.length} ITEMS</span>
                    </div>
                    <div style={{fontSize:12,color:C.textDim,letterSpacing:1,marginBottom:10}}>
                      Every unique drop and which enemy drops it. Use this to target-farm specific items.
                    </div>
                    <div style={{border:`1px solid ${C.border}`}}>
                      {/* Header */}
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",background:"#0c1115",borderBottom:`1px solid ${C.border}`}}>
                        <div style={{padding:"7px 12px",fontSize:11,letterSpacing:2,color:C.dimmer,borderRight:`1px solid ${C.border}`}}>ITEM / DROP</div>
                        <div style={{padding:"7px 12px",fontSize:11,letterSpacing:2,color:C.dimmer}}>FARM FROM</div>
                      </div>
                      {filtered.map(([drop, sources], di) => (
                        <div key={di} style={{display:"grid",gridTemplateColumns:"1fr 1fr",borderBottom:di<filtered.length-1?`1px solid #0f1820`:"none",background:di%2===0?"#090c0e":"#0b0e11"}}
                          onMouseEnter={e=>e.currentTarget.style.background=`${col}0a`}
                          onMouseLeave={e=>e.currentTarget.style.background=di%2===0?"#090c0e":"#0b0e11"}>
                          <div style={{padding:"9px 12px",fontSize:13,color:C.textBright,borderRight:`1px solid #0f1820`,display:"flex",alignItems:"center",gap:6}}>
                            <span style={{width:3,height:3,background:col,flexShrink:0,display:"inline-block"}}/>
                            {drop}
                          </div>
                          <div style={{padding:"9px 12px",fontSize:13,color:C.textDim,lineHeight:1.6,display:"flex",alignItems:"center",flexWrap:"wrap",gap:4}}>
                            {sources.map((src,si) => {
                              const enemy = ENEMIES.find(e=>e.name===src);
                              const tierCol = enemy ? TIER_COLORS[enemy.tier] : C.dim;
                              return (
                                <span key={si} style={{fontSize:11,color:tierCol,background:`${tierCol}15`,border:`1px solid ${tierCol}33`,padding:"1px 6px",whiteSpace:"nowrap"}}>{src}</span>
                              );
                            })}
                          </div>
                        </div>
              ...
              return (
                <div key={tier} style={{marginBottom:20}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
                    <div style={{width:3,height:14,background:col}}/>
                    <span style={{fontSize:15,letterSpacing:4,color:col,fontWeight:700}}>{tier.toUpperCase()}</span>
                    <div style={{flex:1,height:1,background:`${col}22`}}/>
                    <span style={{fontSize:15,color:C.dimmer,letterSpacing:2}}>{tierEnemies.length} ENEMIES</span>
                  </div>
                    ...
                    return (
                      <div key={ei} style={{border:`1px solid ${open?col+"55":C.border}`,marginBottom:6,background:C.panel}}>
                        ...
                        <div onClick={()=>setExpanded(open?null:key)}
                          ...
                          <div style={{flex:1}}>
                            <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",marginBottom:4}}>
                              <span style={{fontSize:15,color:col,fontWeight:900}}>{enemy.name.toUpperCase()}</span>
                              <span style={{fontSize:17,letterSpacing:2,color:col,background:`${col}15`,border:`1px solid ${col}44`,padding:"2px 6px"}}>{tier}</span>
                            </div>
                        ...
                        {open && (
                          <div style={{padding:"12px 14px"}}>
                            {/* Where to find */}
                            <div style={{marginBottom:10}}>
                              <div style={{fontSize:17,letterSpacing:3,color:col,marginBottom:4}}>📍 WHERE TO FIND</div>
                              <div style={{fontSize:16,color:C.text,lineHeight:1.7,background:`${col}08`,padding:"8px 10px",borderLeft:`2px solid ${col}44`}}>{enemy.where}</div>
                            </div>
                            {/* How to kill */}
                            <div style={{marginBottom:10}}>
                              <div style={{fontSize:17,letterSpacing:3,color:C.green,marginBottom:4}}>⚔ HOW TO KILL</div>
                              <div style={{fontSize:16,color:C.text,lineHeight:1.7,background:`${C.green}06`,padding:"8px 10px",borderLeft:`2px solid ${C.green}33`}}>{enemy.how}</div>
                            </div>
                            {/* Ammo + Drops side by side */}
                            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:10}}>
                              <div style={{background:`${C.cyan}08`,padding:"8px 10px",borderLeft:`2px solid ${C.cyan}44`}}>
                                <div style={{fontSize:17,letterSpacing:3,color:C.cyan,marginBottom:4}}>🔫 BEST AMMO / WEAPON</div>
                                <div style={{fontSize:16,color:C.textDim,lineHeight:1.6}}>{enemy.ammo}</div>
                              </div>
                              <div style={{background:`${C.gold}08`,padding:"8px 10px",borderLeft:`2px solid ${C.gold}44`}}>
                                <div style={{fontSize:17,letterSpacing:3,color:C.gold,marginBottom:4}}>📦 DROPS</div>
                                <div style={{display:"flex",flexDirection:"column",gap:3}}>
                                  {enemy.drops.map((d,di)=>(
                                    <span key={di} style={{fontSize:16,color:C.gold}}>{d}</span>
                                  ))}
                                </div>
                              </div>
                            </div>
                            ...
                            <div style={{background:`${C.orange}08`,padding:"8px 10px",borderLeft:`2px solid ${C.orange}44`}}>
                              <div style={{fontSize:17,letterSpacing:3,color:C.orange,marginBottom:4}}>⚡ PRO TIP</div>
                              <div style={{fontSize:16,color:C.textDim,lineHeight:1.6}}>{enemy.tip}</div>
                            </div>
                          </div>
                        )}
                      </div>
            ...
            {ENEMIES.filter(e=>!search||e.name.toLowerCase().includes(search.toLowerCase())||e.drops.join(" ").toLowerCase().includes(search.toLowerCase())).length===0 && (
              <div style={{textAlign:"center",color:C.dim,fontSize:17,padding:30,letterSpacing:3}}>NO RESULTS</div>
            )}
          </>
        )}

        <div style={{marginTop:20,borderTop:`1px solid ${C.border}`,paddingTop:10,fontSize:15,color:C.dimmer,letterSpacing:2,textAlign:"center"}}>
